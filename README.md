# 🛡️ Rosie Domenech

**Cybersecurity Engineer ·**

[![Email](https://img.shields.io/badge/Email-black?style=for-the-badge&logo=gmail)](mailto:ADD_EMAIL_HERE)
![Profile Views](https://komarev.com/ghpvc/?username=RosieDomenech)

---

## 👋 About Me

I’m a cybersecurity engineer working at the intersection of security, infrastructure, and software engineering.

I specialize in threat detection, vulnerability management, and securing enterprise environments- with a focus on turning complex security challenges into practical, scalable solutions.

🎓 Graduate of the NYU Tandon School of Engineering  
📜 Experienced in regulatory and compliance-driven environments  

---

## 🧠 Core Skills

### 🔐 Cybersecurity & SRE
![Splunk](https://img.shields.io/badge/Splunk-black)
![Cortex XDR](https://img.shields.io/badge/Cortex%20XDR-darkgreen)
![Rapid7](https://img.shields.io/badge/Rapid7-red)
![BigFix](https://img.shields.io/badge/BigFix-blue)
![SIEM](https://img.shields.io/badge/SIEM-grey)
![Threat Detection](https://img.shields.io/badge/Threat%20Detection-critical)
![Vulnerability Management](https://img.shields.io/badge/Vulnerability%20Management-orange)

### 💻 Development
![Python](https://img.shields.io/badge/Python-blue)
![JavaScript](https://img.shields.io/badge/JavaScript-yellow)

### ⚙️ Infrastructure & Tools
![AWS](https://img.shields.io/badge/AWS-orange)
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-black)

---

## 🚀 Highlight Projects

| Project | Description |
|--------|------------|
| **STORMGUARD** | Blockchain-based integrity verification for property ownership |
| **Cloud Security Enforcement** | AWS-based detection and remediation using Config, Lambda, SNS |
| **Security Monitoring Dashboards** | Splunk dashboards for asset visibility and threat detection |

---

## 📊 GitHub Stats

<p>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=RosieDomenech&theme=tokyonight" />
  <img src="https://github-readme-stats-sigma-five.vercel.app/api/top-langs/?username=RosieDomenech&layout=compact&theme=tokyonight" />
</p>


---

## ⚡ Focus Areas

- Enterprise Security & Risk Management  
- Cloud Security & Automation  
- Threat Detection & Incident Response  
- AI Applications in Cybersecurity  

---

